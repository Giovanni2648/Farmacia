<?php
	
	require ('../../fpdf/fpdf.php');
	require '../../Config/Conexion.php';
 	$id_vendedor=$_POST['vendedores'];
	 
 	$consulta1 = "SELECT * FROM vendedor WHERE idvendedor = '$id_vendedor'";
    $resultado1 = $conexion->query($consulta1);

    $fila1=$resultado1->fetch_assoc();
    $nom_vendedor=$fila1['nombre'];
	$dir_vendedor=$fila1['direccion'];

   class PDF extends FPDF 
	{
		function Header()
		{
			global $nom_vendedor;
			global $dir_vendedor;
			global $id_planilla;
			global $fecha_planilla;

			//$this->Image('IMG/logoEnte1.jpg',10,8,33);
			$this->Ln(5);
			$this-> SetFont('Arial','',10);
			$this->Cell(40);
			$this-> Cell (150,10,'PLANILLA DE VENDEDORES',1,0,'C',0);
            $this-> Ln(20);
            $this->Cell(100, 10,utf8_decode('Vendedor: '.$nom_vendedor), 2, 0, 'L');
            //comienza encabezado de tabla
            $this-> Ln(20);
			$this-> Cell(30,10,"Nombre",1,0,'C',0);
			$this-> Cell(40,10,"Direccion",1,0,'C',0);
			$this-> Cell(30,10,"Telefono",1,0,'C',0);
            $this-> Ln(10);
		}
		function Footer()
		{
			$this-> SetY(-15);
			$this-> SetFont('Arial','I',8);
			$this-> Cell(0,10,utf8_decode('Página: ').$this->PageNo().'/{nb}',0,0,'C');
		}
	}
	
        
	$consulta = "SELECT * FROM vendedor";

	
	$resultado = $conexion -> query ($consulta);
	$pdf= new PDF('P','mm','A4');
	
	$pdf->AliasNbPages();
    $pdf->AddPage();
    $pdf->SetFont('Arial','',10);
    $con=0;
	while ($fila = $resultado -> fetch_assoc())
		{
			$pdf -> Cell(30,10,$fila['nombre'],1,0,'C',0);
			$pdf -> Cell(40,10,utf8_decode($fila['direccion']),1,0,'C',0);
			$pdf -> Cell(30,10,utf8_decode($fila['telefono']),1,0,'C',0);;
		 }
	$pdf-> Ln(20);
	$pdf->Cell(195,12,utf8_decode('Farmacia - Jujuy'),1,0,'C',0);
	$pdf -> Output();
?>